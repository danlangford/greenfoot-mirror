...
const
    bluejdefsproperty : string = 'greenfoot.windows.vm=';

    jdkregkey : string = '\Software\JavaSoft\Java Development Kit';
    ibmregkey : string = '\Software\IBM\Java Development Kit';
    greenfootregkey : string = '\Software\Greenfoot\Greenfoot\1.4.1';

    searchingstartcaption : string = 'Search drives for all Java versions...';
    searchingstopcaption : string = 'Stop Search';

    foundjavacaption1 : string = 'Greenfoot has found more than one Java version that can be used.';
    foundjavacaption2 : string = 'Please select one and select "Launch" to use it with Greenfoot or';
    foundjavacaption3 : string = 'select "Advanced" if you wish to search for other Java versions';

    foundonejavacaption1 : string = 'Greenfoot has found the following Java version that can be used.';
    foundonejavacaption2 : string = 'Please select "Launch" if you wish to use it with Greenfoot or';
    foundonejavacaption3 : string = 'select "Advanced" if you wish to look for other Java versions';

    nojavacaption1 : string = 'Greenfoot could not find any Java systems. A JDK/J2SDK must be';
    nojavacaption2 : string = 'installed to run Greenfoot. If one is installed on your system,';
...
type
  ...
  private
        ...
        { command line flag to tell Greenfoot to display the VM selector
          no matter what }
        forcedialog : boolean;

        { flag indicating that we have already launched Greenfoot, so
        ...
        function LaunchGreenfoot(jdkpath, ver : string) : boolean;
...
begin
    ...
    try
        ...
        if reg.OpenKey(greenfootregkey, false) then
        begin
    ...
    case goodvmsfound.Count of
     ...
     1:
        begin
            ...
            if (not forcedialog) then
            begin
	            LaunchGreenfoot(GoodVM.Items[0].Caption, GoodVM.Items[0].SubItems[0] );
                Close;
            end;
        end;
    else
...
{
    Launch Greenfoot using 'jdkpath' as the location of the
    VM to use.
}
function TMainForm.LaunchGreenfoot(jdkpath, ver : string) : boolean;
var
        appdir, appdirlib, vmfilename,
          bluejjarfilename , toolsjarfilename : string;
        exfile : TExFile;
begin
    { lets never launch Greenfoot twice }
    if launched then
    begin
        result := false;
        Exit;
    ...
    //if usejavaw then
    //else
    if StrLIComp(PChar(ver), '"1.5', 4) >= 0 then
...
//        goodparams.Add('-Dbluej.language=' + LanguageComboBox.Items[LanguageComboBox.ItemIndex]);
    ...
    exfile.ProcParameters := '-classpath "' + bluejjarfilename + ';'
              + appdirlib + 'junit.jar' + ';'
              + appdirlib + 'bluejcore.jar' + ';'
              + appdirlib + 'bluejext.jar' + ';'
              + appdirlib + 'bluejeditor.jar' + ';'
              + appdirlib + 'antlr.jar' + ';'
              + toolsjarfilename + ';'
              + appdirlib + 'MRJ141Stubs.jar' + ';'
              + appdirlib + 'extensions\greenfoot.jar'
              + '" bluej.Boot -greenfoot=true -bluej.compiler.showunchecked=false'
...
begin
	...
	LaunchGreenfoot(GoodVM.Selected.Caption, GoodVM.Selected.SubItems[0]);

    reg := TRegistry.Create;
    try
        reg.RootKey := HKEY_CURRENT_USER;

        if reg.OpenKey(greenfootregkey, true) then
        begin
